...
int main(int argc, char* argv[])
{
    ...
    printf("-------------------------------------------------------------------------\n");
    std::string filename = (argc > 1) ? argv[1] : "../data/Rigged Hand.fbx";
    ...
    for (unsigned int i = 0; i < scene->mNumMeshes; ++i) {
        ...
        printf(
            ...
            scene->mMeshes[i]->mAABB.mMin.x,
            scene->mMeshes[i]->mAABB.mMin.y,
            scene->mMeshes[i]->mAABB.mMin.z,
            scene->mMeshes[i]->mAABB.mMax.x,
    ...
    for (unsigned int i = 0; i < scene->mNumLights; ++i) {
        ...
        case aiLightSource_DIRECTIONAL: // Направенный свет
            ...
            printf("    UpDirection = (%f, %f, %f)\n",
                scene->mLights[i]->mUp.x,
                scene->mLights[i]->mUp.y,
                scene->mLights[i]->mUp.z
            );
            printf("    Direction   = (%f, %f, %f)\n",
                scene->mLights[i]->mDirection.x,
                scene->mLights[i]->mDirection.y,
                scene->mLights[i]->mDirection.z
            );
            break;
        case aiLightSource_POINT: // Точечный источник света с интенсивностью 1/(att0 + att1*d + att2*d^2)
            printf("    Type     = %d = POINT\n", scene->mLights[i]->mType);
            ...
            break;
        case aiLightSource_SPOT: // Коническая лампочка, максимум в InnerCone, за пределами OuterCone интенсивность 0, внутри - гладкая интерполяция
            printf("    Type     = %d = SPOT\n", scene->mLights[i]->mType);
            printf("    Position = (%f, %f, %f)\n",
                scene->mLights[i]->mPosition.x,
                scene->mLights[i]->mPosition.y,
        ...
        case aiLightSource_AREA: // Светящийся прямоугольник
            ...
            printf("    Size = (%f, %f)\n",
                scene->mLights[i]->mSize.x,
                scene->mLights[i]->mSize.y
            );
            break;
        }
    ...
    std::vector<std::string> typesOfProperties = {
        "undefined",
        "aiPTI_Float",
        "aiPTI_Double",
        "aiPTI_String",
    ...
    for (unsigned int i = 0; i < scene->mNumMaterials; ++i) {
        ...
        for (unsigned int j = 0; j < scene->mMaterials[i]->mNumProperties; ++j) {
            ...
            case (aiPTI_String):
                printf(
                    "        Strange Data (Device Control Symbols in ASCII) = %d %d\n", 
                    scene->mMaterials[i]->mProperties[j][0].mData[0], 
                    scene->mMaterials[i]->mProperties[j][0].mData[1]
                );
                printf("        Data = \"");
    ...
    //exporter.Export(scene, "gltf2", "data/export.gltf");
    return 0;
}